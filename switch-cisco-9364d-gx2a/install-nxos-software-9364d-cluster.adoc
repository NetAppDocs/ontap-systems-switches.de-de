...
summary: Mit diesem Verfahren können Sie die NX-OS-Software auf dem Nexus 9364d-gx2a-Cluster-Switch installieren oder aktualisieren. 
---
...
[role="lead"]
Befolgen Sie dieses Verfahren, um die NX-OS-Software auf dem Nexus 9364D-GX2A-Switch zu installieren oder zu aktualisieren.
...
* Führen Sie die Prozedur inlink:install-nxos-overview-9364d-cluster.html["Bereiten Sie die Installation von NX-OS und RCF vor."] Die
* Ein aktuelles Backup der Switch-Konfiguration ist verfügbar.
* Es ist ein voll funktionsfähiger Cluster vorhanden (keine Fehler in den Protokollen oder ähnliche Probleme).
...
. Kopieren Sie die NX-OS-Software und EPLD-Images auf den Nexus 9364D-GX2A-Switch.
+
.Beispiel anzeigen
[%collapsible]
====
...
.Wie geht es weiter?
Nach der Installation oder Aktualisierung der NX-OS-Software können Sie link:install-upgrade-rcf-overview-cluster-9364d.html["Installieren oder aktualisieren Sie die Referenzkonfigurationsdatei (RCF)."].
