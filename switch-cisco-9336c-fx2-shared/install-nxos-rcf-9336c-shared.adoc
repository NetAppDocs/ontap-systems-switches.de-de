...
[role="lead"]
Sie können die Referenzkonfigurationsdatei (RCF) installieren, nachdem Sie den Switch Nexus 9336C-FX2 zum ersten Mal eingerichtet haben.

Bevor Sie beginnen, führen Sie bitte die folgende Prozedur durch:link:prepare-nxos-rcf-9336c-shared.html["Bereiten Sie die Installation von NX-OS und RCF vor."] Die
...
* Inter-Switch-Link-Verbindungen (ISL) zwischen den Switches sind hergestellt.
* Die Ports des ONTAP Knotenclusters sind nicht verbunden.


.Verfügbare RCF-Konfiguration
* *ClusterStorageRCF* - Unterstützt einen partitionierten Cluster plus zwei Speicherzonen auf den Switches (Cluster-Storage RCF 1._xx_).
...
. Wiederholen Sie die Schritte 1 bis 8 auf Switch cs2.
...
.Wie geht es weiter?
Nach der Installation des RCF können Sie link:../switch-cshm/config-overview.html["Konfigurieren der Switch-Integritätsüberwachung"]Die
