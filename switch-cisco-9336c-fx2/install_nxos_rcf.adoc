...
* Bevor Sie dieses Verfahren durchführen, stellen Sie sicher, dass eine aktuelle Sicherung der Switch-Konfiguration vorhanden ist.
...
----
. Überprüfen Sie, ob die Cluster-LIFs zu den Ports migriert wurden, die auf Cluster-Switch cs1 gehostet werden. Dies kann einige Sekunden dauern.`network interface show -role cluster`
+
[listing]
----
cluster1::*> network interface show -role cluster
