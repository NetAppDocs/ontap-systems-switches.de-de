...
Bevor Sie beginnen, führen Sie bitte die folgende Prozedur durch:link:install-nxos-overview-9336c-cluster.html["Bereiten Sie die Installation von NX-OS und RCF vor."] Die
...
----
====
. Aktualisieren Sie das EPLD-Image und starten Sie den Switch neu. Wenn Sie NX-OS 10.5(3)F oder höher verwenden, müssen Sie kein separates EPLD-Image-Upgrade durchführen, da das EPLD-Image im NX-OS-Image enthalten ist. Überspringen Sie diesen Schritt und fahren Sie mit der Überprüfung der Cluster-Port-Gesundheit fort.
+
.Beispiel anzeigen
[%collapsible]
====
...
.Wie geht es weiter?
Nach der Installation oder Aktualisierung der NX-OS-Software können Sie link:install-upgrade-rcf-overview-cluster-9336c.html["Installieren oder aktualisieren Sie die Referenzkonfigurationsdatei (RCF)."].
