...
Der Ethernet Switch Health Monitor (CSHM) nutzt SNMP zur Überwachung des Zustands und der Leistung von Netzwerk-Switches. Standardmäßig wird SNMPv2c automatisch über die Referenzkonfigurationsdatei (RCF) konfiguriert. SNMPv3 ist sicherer als SNMPv2, da es robuste Sicherheitsfunktionen wie Authentifizierung, Verschlüsselung und Nachrichtenintegrität einführt, die vor unberechtigtem Zugriff schützen und die Vertraulichkeit sowie Integrität der Daten während der Übertragung gewährleisten.
