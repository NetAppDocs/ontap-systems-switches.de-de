...
[NOTE]
====
. Regenerieren Sie SSH-Schlüssel auf dem Switch gemäß den Anweisungen des Anbieters.
. Regenerieren Sie SSH-Schlüssel auf der ONTAP-Seite mit `debug system regenerate-systemshell-key-pair`
. Führen Sie die Setup-Routine für die Protokollsammlung mithilfe von erneut aus `system switch ethernet log setup-password`
...
* Bei NVIDIA-Switches muss es dem Benutzer für die Protokollerfassung erlaubt sein, die Protokollerfassungsbefehle auszuführen, ohne dass eine Kennwortaufforderung angezeigt wird. Führen Sie den folgenden Befehl aus, um diese Verwendung zuzulassen:
