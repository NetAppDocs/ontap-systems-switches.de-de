...
summary: 'Die Funktion zur Protokollerfassung des Ethernet-Switch-Integritätsmonitors (CSHM) wird verwendet, um Protokolle von Cluster- oder Speichernetzwerk-Switches in ONTAP zu sammeln.' 
---
...
[role="lead"]
Der Ethernet-Switch-Integritätsmonitor (CSHM) ist für die Überwachung des Betriebszustands von Netzwerk-Switches und die Erfassung von Switch-Protokollen zu Debugging-Zwecken zuständig. Diese Anleitung führt Sie durch die Einrichtung der Protokollerfassung, die Anforderung detaillierter *Support*-Protokolle und die Aktivierung der stündlichen Erfassung von *periodischen* Daten, die von AutoSupport erfasst werden.
...
CAUTION: Falls von der Protokollerfassungsfunktion Fehlerstatus gemeldet werden (sichtbar in der Ausgabe von  `system switch ethernet log show`), siehe link:log-collection-troubleshoot.html["Fehlerbehebung bei der Protokollerfassung"] für weitere Details.
