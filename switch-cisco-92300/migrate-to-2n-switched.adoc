...
NOTE: Nach Abschluss Ihrer Migration müssen Sie möglicherweise die erforderliche Konfigurationsdatei installieren, um den Cluster Switch Health Monitor (CSHM) für 92300YC-Cluster-Switches zu unterstützen. Sehen link:../switch-cshm/cshm-overview.html["Switch-Zustandsüberwachung (CSHM)"] .
