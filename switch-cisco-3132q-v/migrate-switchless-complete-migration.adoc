...
.Schritte
. Zeigen Sie die Cluster-Switch-Portnummern an, mit denen jeder Cluster-Port auf jedem Knoten verbunden ist:
+
...
----
====
. Deaktivieren Sie die Einstellungen für die schalterlose Zwei-Knoten-Konfiguration auf einem beliebigen Knoten:
+
...
----
. Überprüfen Sie, ob die `switchless-cluster` Diese Option wurde deaktiviert.
+
[listing]
----
