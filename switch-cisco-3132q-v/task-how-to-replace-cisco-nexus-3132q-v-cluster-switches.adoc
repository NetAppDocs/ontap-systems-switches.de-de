...
---
= So ersetzen Sie Cisco Nexus 3132Q-V Cluster-Switches
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Der Austausch eines defekten Cisco Nexus 3132Q-V Switches in einem Cluster-Netzwerk erfolgt durch die unterbrechungsfreie Durchführung (NDO), und Sie müssen eine bestimmte Sequenz von Aufgaben durchführen.

.Bevor Sie beginnen
* Die vorhandene Cluster- und Netzwerkkonfiguration muss über folgende Merkmale verfügen:
+
** Die Nexus 3132Q-V Cluster-Infrastruktur muss redundant und auf beiden Switches voll funktionsfähig sein.
+
...
** Alle logischen Cluster-Schnittstellen (LIFs) müssen sich im befinden `up` Staat und darf nicht migriert worden sein.


* Der Nexus 3132Q-V-Ersatzschalter:
+
...
** Der gewünschte RZF- und NX-OS-Betriebssystem-Bildschalter muss auf den Switch geladen werden.
...
.Über diese Aufgabe
Bei diesem Verfahren wird der zweite Nexus 3132Q-V Cluster Switch CL2 durch den neuen 3132Q-V Switch C2 ersetzt. Die Beispiele in diesem Verfahren verwenden die folgende Nomenklatur für Switches und Knoten:
...
* Cluster-Switch CL2 wird durch C2 ( ersetzt<<step1_replace3132q,Schritt 1>> -<<step21_replace3132q,21>>)
+
...
.Schritte
. [[ste1_ersetze3132q]]Wenn AutoSupport in diesem Cluster aktiviert ist, unterdrücken Sie die automatische Erstellung eines Cases durch Aufrufen einer AutoSupport Meldung:
+
...
. ISLs-Ports 1/31 und 1/32 auf dem Nexus 3132Q-V Switch CL1 heraufholen.
+
[listing]
----
...
1     Po1(SU)      Eth    LACP      Eth1/31(P)   Eth1/32(P)
----
. Fahren Sie auf allen Knoten alle mit dem Nexus 3132Q-V Switch verbundenen Cluster-Interconnect-Ports C2: `network port modify`
+
[listing]
----
...
----
. [[step21_ersetze3132q]]Entfernen Sie den ausgetauschten Nexus 3132Q-V Schalter, wenn er nicht bereits automatisch entfernt wird:
+
