...
---
= So ersetzen Sie einen Cisco Nexus 5596 Cluster Switch durch einen Cisco Nexus 3132Q-V Cluster Switch
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Über diese Aufgabe
Die Beispiele in diesem Verfahren beschreiben den Austausch von Nexus 5596 Switches durch Nexus 3132Q-V Switches. Mit diesen Schritten (durch Änderungen) können andere ältere Cisco Switches ersetzt werden. Für das Verfahren wird die folgende Nomenklatur von Switches und Nodes verwendet:
...
* Der zu ersetzende Cluster-Switch CL2 durch C2 (<<step20_replace5596_3232q,Schritt 20>> - <<step33_replace5596_3232q,33>>)
+
...
* Zwei FAS9000 Nodes wurden dem Cluster hinzugefügt, wobei Beispiele für Cluster-Details ( zeigen<<step34_replace5596_3232q,Schritt 34>> - <<step37_replace5596_3232q,37>>).


.Schritte
. [[ste1_ersetzung5596_32q]]Wenn AutoSupport für diesen Cluster aktiviert ist, unterdrücken Sie die automatische Fallerstellung durch Aufrufen einer AutoSupport Meldung: `system node autosupport invoke -node * -type all -message MAINT=xh`
+
...
----
. Fahren Sie die ISL-Ports 41 bis 48 am aktiven Nexus 5596-Switch CL1 herunter:
+
...
----
+
Wenn Sie einen Nexus 5010 oder 5020 ersetzen, geben Sie die entsprechenden Portnummern für ISL von Seite 1 an.
...
----
. [[step19_ersetzung5596_32q]]]Alle migrierten Cluster-Interconnect-LIFs, die mit C2 verbunden sind, auf allen Nodes zurücksetzen:
+
...
----
. [[step20_ersetze5596_32q]]Überprüfen Sie, ob alle Cluster-Interconnect-Ports jetzt auf ihr Zuhause zurückgesetzt werden:
+
`network interface show`
...
----
. [[step33_ersetze5596_32q]]Überprüfen Sie den Status des Cluster-Node-Ports: `network port show`
+
...
8 entries were displayed.
----
. [[step34_ersetzung5596_3232q]]setzen Sie auf allen Nodes die spezifischen Cluster-LIFs auf ihre Home-Ports zurück:
+
`network interface revert`
...
4 paths up, 0 paths down (udp check)
----
. [[Schritt37_ersetzung5596_32q]]erweitern Sie das Cluster, indem Sie den Nexus 3132Q-V Cluster Switches Nodes hinzufügen.
. Zeigen Sie die Informationen zu den Geräten in Ihrer Konfiguration an:
+
** `network device-discovery show`
** `network port show -role cluster`
** `network interface show -role cluster`
** `system cluster-switch show`+ die folgenden Beispiele zeigen die Nodes n3 und n4 mit 40-GbE-Cluster-Ports, die mit den Ports e1/7 bzw. e1/8 verbunden sind, sowohl auf den Nexus 3132Q-V Cluster-Switches, und beide Nodes haben sich dem Cluster angeschlossen. Die 40 GbE Cluster Interconnect Ports sind e4a und e4e.
+
[listing]
----
cluster::> network device-discovery show
...
----
. Entfernen Sie den ausgetauschten Nexus 5596, wenn sie nicht automatisch entfernt werden: `system cluster-switch delete`
+
