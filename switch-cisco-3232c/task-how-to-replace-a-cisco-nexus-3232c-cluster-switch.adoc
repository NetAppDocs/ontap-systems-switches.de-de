...
[role="lead"]
Ein defekter Cisco Nexus 3232C Switch kann in einem Cluster unterbrechungsfrei durch Ausführen einer bestimmten Aufgabenfolge ausgetauscht werden.

.Bevor Sie beginnen
Die vorhandene Cluster- und Netzwerkkonfiguration muss folgende Merkmale aufweisen:

* Die Nexus 3232C-Cluster-Infrastruktur muss auf beiden Switches redundant und voll funktionsfähig sein.
+
...
* Alle logischen Cluster-Schnittstellen (LIFs) müssen den Status „up“ aufweisen und dürfen nicht migriert werden.
...
* *Anzeige und Migration der Cluster-Ports auf Switch C2 (Schritte <<one,1>>-<<seven,7)>>*
* *ISL-Kabel vom Switch CL2 wieder an Switch C2 anschließen und ISLs auf Switch CL1 und C2 migrieren (Schritte <<eight,8>>-<<fourteen,14)>>*
* *Zurücksetzen aller LIFs auf die ursprünglich zugewiesenen Ports (Schritte <<fifteen,15>>-<<eighteen,18)>>*
* *Überprüfung, ob alle Ports und LIF korrekt migriert wurden (Schritte <<nineteen,19>>-<<twentyone,21>>)*


Bei diesem Verfahren wird der zweite Nexus 3232C-Cluster-Switch CL2 durch den neuen 3232C-Switch C2 ersetzt. Die Beispiele in diesem Verfahren verwenden die folgende Nomenklatur für Switches und Knoten:
...
Die Beispiele in diesem Verfahren verwenden vier Knoten. Zwei der Nodes verwenden vier 10 GB Cluster Interconnect Ports: e0a, e0b, e0c und e0d. Die anderen beiden Knoten verwenden zwei 40 GB Cluster Interconnect Ports: e4a und e4e. Siehe link:https://hwu.netapp.com/SWITCH/INDEX.html["Hardware Universe"^] Um zu überprüfen, welche Cluster-Ports für Ihre Plattform korrekt sind.

Dieses Verfahren beschreibt das folgende Szenario:
...
12 entries were displayed.
----
. Fahren Sie die Cluster-Interconnect-Ports herunter, die physisch mit dem ursprünglichen Switch CL2 verbunden sind: `network port modify -node _node-name_ -port _port-name_ -up-admin false`
+
...
----
. [[sieben]]Ping die Remote-Cluster-Schnittstellen und führen Sie eine RPC-Server-Prüfung durch:
+
...
----
. [[acht]]Schalten Sie die Ports 1/31 und 1/32 am Cluster-Switch CL1 aus.
+
...
1     Po1(SU)      Eth    LACP      Eth1/31(P)   Eth1/32(P)
----
. [[vierzehn]]auf allen Knoten alle Cluster-Interconnect-Ports, die mit dem Ersatz-Switch verbunden sind, aufrufen C2: `network port modify -node _node-name_ -port _port-name_ -up-admin true`
+
[listing]
----
...
----
. [[fünfzehn]]]Zurücksetzen aller migrierten Cluster-Interconnect-LIFs auf allen Nodes:
+
...
e4e       Cluster      Cluster          up   9000 auto/40000  -
        12 entries were displayed.
----
. [[achtzehn]]Ping the Remote Cluster Interfaces and Perform an RPC Server Check:
+
`cluster ping-cluster -node _node-name_`
...
8	paths up, 0 paths down (udp check)
----
. [[neunzehn]]Zeigen Sie die Informationen über die Geräte in Ihrer Konfiguration, indem Sie die folgenden Befehle eingeben:
+
...
. [[twentyone]]Überprüfen Sie, ob die richtigen Cluster-Switches überwacht werden: `system cluster-switch show`
+
