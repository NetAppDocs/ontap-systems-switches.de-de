...
---
= Migrieren eines CN1610 Cluster-Switches zu einem Cisco Nexus 3232C Cluster-Switch
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
In der folgenden Liste werden die Phasen beschrieben, die Sie beim Ändern der Cluster-Switches durchführen müssen:

* ICH Ersetzen Sie den Cluster-Switch CL2 durch C2 (Schritte) <<one,1>>-<<twentytwo,22>>)
* II Ersetzen Sie den Cluster-Switch CL1 durch C1 (Schritte) <<twentythree,23>>-<<forty,40>>)
...
+
.. Zeigt die Attribute des Cluster-Netzwerk-Ports an: `network port show -role cluster`
+
[listing]
----
...
8 entries were displayed.
----
.. Informationen zu den logischen Schnittstellen anzeigen: `network interface show -role cluster`
+
[listing]
----
...
----
. Überprüfen Sie den Systemzustand des Clusters: `network interface show -role cluster`
+
[listing]
----
cluster::*> network interface show -role cluster
...
+
Diese Konfiguration stimmt mit der ISL-Konfiguration auf dem 3232C-Switch C2 überein, wenn die ISLs in Schritt 10 auf beiden Switches hochgeschaltet werden.
...
----
. Überprüfen Sie, ob es sich bei den ISLs um handelt `up` Am 3232C-Switch C2: `show port-channel summary`
+
Weitere Informationen zu Cisco-Befehlen finden Sie in den Handbüchern im https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-command-reference-list.html["Referenzen für NX-OS-Befehle der Cisco Nexus 3000-Serie"^].
...
2	    Po2(SU)     Eth      LACP      Eth1/24/1(P) Eth1/24/2(P) Eth1/24/3(P)
                                     Eth1/24/4(P)
----
. Alle Cluster-Interconnect-Ports, die auf allen Knoten mit dem 3232C-Switch C2 verbunden sind, werden verfügbar: `network port modify -node _node-name_ -port _port-name_ -up-admin true`
+
...
----
. Zurücksetzen aller migrierten Cluster-Interconnect-LIFs, die auf allen Nodes mit C2 verbunden sind: `network interface revert -vserver cluster -lif _lif-name_`
+
[listing]
----
...
----
. Vergewissern Sie sich, dass alle Cluster-Interconnect-Ports auf die Home-Ports zurückgesetzt werden: `network interface show -role cluster`
+
...
8 entries were displayed.
----
. Vergewissern Sie sich, dass alle Cluster-Ports verbunden sind: `network port show -role cluster`
+
...
8 entries were displayed.
----
. Pingen Sie die Remote-Cluster-Schnittstellen und führen Sie dann eine Remote-Prozedur aus Rufen Sie den Server an: `cluster ping-cluster -node _node-name_`
+
Im folgenden Beispiel wird Node n1 beflügelt und der RPC-Status danach angezeigt:
...
4 paths up, 0 paths down (udp check)
----
. [[twentytwo]] Migrieren Sie die LIFs, die mit dem ersten CN1610-Switch CL1 verknüpft sind: `network interface migrate -vserver cluster -lif _lif-name_ -source-node _node-name_`
+
...
----
. [[twentythree]]Status des Clusters überprüfen: `network interface show -role cluster`
+
...
8 entries were displayed.
----
. Fahren Sie die Node-Ports, die auf allen Nodes mit CL1 verbunden sind, herunter: `network port modify -node _node-name_ -port _port-name_ -up-admin false`
+
...
1     Po1(SU)     Eth      LACP      Eth1/31(P)   Eth1/32(P)
----
. Alle mit dem neuen 3232C-Switch C1 verbundenen Cluster-Interconnect-Ports auf allen Knoten: `network port modify -node _node-name_ -port _port-name_ -up-admin true`
+
...
----
. Überprüfen Sie den Status des Cluster-Node-Ports: `network port show -role cluster`
+
...
8 entries were displayed.
----
. Zurücksetzen aller migrierten Cluster-Interconnect-LIFs, die ursprünglich auf allen Knoten mit C1 verbunden waren: `network interface revert -server cluster -lif _lif-name_`
+
Sie müssen jede LIF individuell wie im folgenden Beispiel gezeigt migrieren:
...
----
. Vergewissern Sie sich, dass die Schnittstelle jetzt die Startseite ist: `network interface show -role cluster`
+
...
8 entries were displayed.
----
. Pingen Sie die Remote-Cluster-Schnittstellen und führen Sie dann eine Remote-Prozedur aus Rufen Sie den Server an: `cluster ping-cluster -node _host-name_`
+
Im folgenden Beispiel wird Node n1 beflügelt und der RPC-Status danach angezeigt:
...
** `system cluster-switch show`+ die folgenden Beispiele zeigen die Nodes n3 und n4 mit 40-GbE-Cluster-Ports, die mit den Ports e1/7 bzw. e1/8 verbunden sind, auf beiden Nexus 3232C-Cluster-Switches. Beide Nodes sind dem Cluster verbunden. Die 40 GbE Cluster Interconnect Ports sind e4a und e4e.
...
----
. Entfernen Sie die ausgetauschten CN1610-Schalter, wenn sie nicht automatisch entfernt werden: `system cluster-switch delete -device _switch-name_`
+
...
----
. [[vierzig]]Aktivieren Sie die Protokollerfassung für die Cluster Switch-Systemzustandsüberwachung zum Erfassen von Switch-bezogenen Protokolldateien: `system cluster-switch log setup-password`
+
