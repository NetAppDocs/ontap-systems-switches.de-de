...
summary: Sie können das RCF nach der Konfiguration des BES-53248 Netzwerk-Switches und nach Anwendung aller neuen Lizenzen aktualisieren. 
---
= Aktualisieren Sie die RCF-Version
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sie können das RCF nach dem Upgrade des BES-53248 Cluster-Switch EFOS und nach dem Anwenden aller neuen Lizenzen aktualisieren.
...
* Weitere Informationen zu den Upgrade- und Downgrade-Verfahren des BES-53248-Switches finden Sie in den entsprechenden Software- und Upgrade-Anleitungen auf der link:https://www.broadcom.com/support/bes-switch["Broadcom"^] Website.
...
+
.. Wenn auf dem Switch zusätzliche Portlizenzen installiert wurden, müssen Sie die RCF anpassen, um die zusätzlichen lizenzierten Ports zu konfigurieren. Siehe link:configure-licenses.html#activate-newly-licensed-ports["Aktivieren Sie neu lizenzierte Ports"] für Details. Wenn Sie jedoch auf RCF 1.12 oder höher aktualisieren, sind diese Anpassungen nicht mehr erforderlich, da alle Schnittstellen nun vorkonfiguriert sind.
...
CAUTION: Die Datei muss beim Hochladen und Anwenden der RCF-Datei  `reference-config.scr`benannt werden, andernfalls schlägt der Anwendungsschritt des Skripts fehl.

. Kopieren Sie die RCF auf den Switch cs2 mithilfe eines der folgenden Übertragungsprotokolle: HTTP, HTTPS, FTP, SFTP oder SCP.
+
...
----
+
Weitere Einzelheiten finden Sie unter link:https://docs.netapp.com/us-en/ontap/networking/revert_a_lif_to_its_home_port.html["LIF zum Heimathafen zurückversetzen"].
