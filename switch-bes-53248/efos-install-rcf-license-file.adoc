...
---
= Installieren Sie die Referenz-Konfigurationsdatei (RCF) und die Lizenzdatei
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Ab EFOS 3.12.0.1 können Sie die RCF- und Lizenzdatei nach der Konfiguration des BES-53248 Cluster-Switch installieren.


CAUTION: Alle Ports werden bei der Installation des RCF konfiguriert, aber Sie müssen Ihre Lizenz installieren, um die konfigurierten Ports zu aktivieren.
...
* Die aktuelle RCF, verfügbar auf der link:https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["Broadcom Cluster-Switches"^] Seite.
...
* Die Beispiele in diesem Verfahren verwenden vier Knoten. Diese Knoten nutzen zwei 10GbE-Cluster-Verbindungsports `e0a` und `e0b`. Siehe link:https://hwu.netapp.com/Home/Index["Hardware Universe"^], um die korrekten Cluster-Ports auf Ihren Plattformen zu überprüfen.
...
. Kopieren Sie die RCF-Datei mithilfe eines der folgenden Übertragungsprotokolle in den Bootflash des Switches cs2: FTP, HTTP, TFTP, SFTP oder SCP.
+
...
----
====
. Überprüfen Sie am Schalter, ob Ihre Änderungen vorgenommen wurden:
+
[source, cli]
----
show running-config
...
--
====
. [[step21]]Schalten Sie auf dem Cluster-Switch cs1 die mit den Cluster-Ports der Knoten verbundenen Ports ab.
+
...
.Wie geht es weiter?
Nach der Installation der RCF- und Lizenzdatei können Sie link:configure-ssh.html["SSH aktivieren"]Die
