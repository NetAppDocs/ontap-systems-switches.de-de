...
summary: 'Sie können die Referenzkonfigurationsdatei (RCF) installieren, nachdem Sie den Cluster-Switch BES-53248 konfiguriert und die neuen Lizenzen angewendet haben.' 
---
= Installieren Sie die Referenz-Konfigurationsdatei (RCF)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sie können das RCF installieren, nachdem Sie den BES-53248 Cluster-Switch konfiguriert und die neuen Lizenzen angewendet haben.


CAUTION: Für EFOS 3.12 und höher befolgen Sie die Installationsschritte in link:efos-install-rcf-license-file.html["Installieren Sie die Referenz-Konfigurationsdatei (RCF) und die Lizenzdatei"].
...
* Die aktuelle RCF-Datei, verfügbar auf der link:https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["Broadcom Cluster-Switches"^] Seite.
...
* Eine Konsolenverbindung zum Switch ist erforderlich, wenn die RCF aus dem Werkszustand installiert wird. Diese Anforderung ist optional, wenn Sie den Knowledge Base Artikel link:https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["Wie man die Konfiguration eines Broadcom-Interconnect-Switches löscht und gleichzeitig die Remote-Konnektivität beibehält"^] verwendet haben, um die Konfiguration zuvor zu löschen.


.Empfohlene Dokumentation
Konsultieren Sie die Switch-Kompatibilitätstabelle für die unterstützten ONTAP- und RCF-Versionen. Siehe link:https://mysupport.netapp.com/site/info/broadcom-cluster-switch["EFOS-Software-Download"^]. Beachten Sie, dass es Befehlsabhängigkeiten zwischen der Befehlssyntax im RCF und der in EFOS-Versionen geben kann.
...
* Die Beispiele in diesem Verfahren verwenden vier Knoten. Diese Knoten nutzen zwei 10GbE-Cluster-Verbindungsports `e0a` und `e0b`. Siehe die link:https://hwu.netapp.com/Home/Index["Hardware Universe"^] zur Überprüfung der korrekten Cluster-Ports auf Ihren Plattformen.
...
CAUTION: Bevor Sie eine neue Switch-Softwareversion und RCFs installieren, verwenden Sie den Knowledge Base Artikel link:https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["Wie man die Konfiguration eines Broadcom-Interconnect-Switches löscht und gleichzeitig die Remote-Konnektivität beibehält"^]. Wenn Sie die Switch-Einstellungen vollständig löschen müssen, müssen Sie die Basiskonfiguration erneut durchführen. Sie müssen über die serielle Konsole mit dem Switch verbunden sein, da ein vollständiges Löschen der Konfiguration die Konfiguration des Management-Netzwerks zurücksetzt.
...
show isdp neighbor
----
. Schalten Sie auf dem Cluster-Switch cs2 die Ports ab, die mit den Cluster-Ports der Knoten verbunden sind.  Wenn beispielsweise die Ports 0/1 bis 0/16 mit ONTAP Knoten verbunden sind:
+
[listing, subs="+quotes"]
----
(cs2)> *enable*
...
+
.. Wenn auf dem Switch zusätzliche Portlizenzen installiert wurden, müssen Sie die RCF anpassen, um die zusätzlichen lizenzierten Ports zu konfigurieren. Siehe link:configure-licenses.html#activate-newly-licensed-ports["Aktivieren Sie neu lizenzierte Ports"] für Details.
...
--
====
. Untersuchen Sie die Bannerausgabe von `show clibanner` Befehl.  Sie müssen diese Anweisungen lesen und befolgen, um die korrekte Konfiguration und Funktion des Schalters zu gewährleisten.
+
.Beispiel anzeigen
[%collapsible]
====
...
----
. Auf dem Cluster-Switch cs2 werden die mit den Cluster-Ports der Knoten verbundenen Ports aktiviert.  Wenn beispielsweise die Ports 0/1 bis 0/16 mit ONTAP Knoten verbunden sind:
+
[listing, subs="+quotes"]
----
(cs2)> *enable*
...
--
====
. [[step20]]Schalten Sie auf dem Cluster-Switch cs1 die mit den Cluster-Ports der Knoten verbundenen Ports ab.
+
...
----
. Neustart des Switches cs1.  Dies veranlasst die Cluster-LIFs, zu ihren ursprünglichen Ports zurückzukehren.  Sie können die auf den Knoten gemeldeten Ereignisse „Cluster-Ports ausgefallen“ ignorieren, während der Switch neu startet.
+
[listing, subs="+quotes"]
----
(cs1)# *reload*
...
.Wie geht es weiter?
Nach der Installation des RCF können Sie link:configure-ssh.html["SSH aktivieren"]Die
