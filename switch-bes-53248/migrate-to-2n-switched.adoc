...
Nach Abschluss der Migration müssen Sie möglicherweise die erforderliche Konfigurationsdatei installieren, um den Ethernet Switch Health Monitor (CSHM) für BES-53248-Cluster-Switches zu unterstützen. Siehe link:CSHM_log_collection.html["Aktivieren Sie die Protokollerfassung"].
