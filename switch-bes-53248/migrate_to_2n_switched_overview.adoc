...
summary: Wenn Sie eine vorhandene Cluster-Umgebung mit zwei Nodes ohne Switches nutzen, können Sie mit den von Broadcom unterstützten BES-53248 Cluster-Switches zu einer Switch-basierten 2-Node-Umgebung migrieren. So können Sie eine Skalierung auf mehr als zwei Nodes im Cluster vornehmen. 
---
= Migration zu einer Switch-basierten NetApp Cluster-Umgebung mit von Broadcom unterstützten BES-53248 Cluster-Switches
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Wenn Sie eine vorhandene Cluster-Umgebung mit zwei Nodes ohne Switches nutzen, können Sie mit den von Broadcom unterstützten BES-53248 Cluster-Switches zu einer Switch-basierten 2-Node-Umgebung migrieren. So können Sie eine Skalierung auf mehr als zwei Nodes im Cluster vornehmen.
...
* Auf den Knoten muss ONTAP 9.5P8 und höher ausgeführt werden. Die Unterstützung für 40/100-GbE-Cluster-Ports beginnt mit der EFOS-Firmware-Version 3.4.4.6 und höher.
...
Von Broadcom unterstützte BES-53248 Cluster-Switch-Konfiguration:

* Der BES-53248-Cluster-Switch muss bei beiden Switches voll funktionsfähig sein.
...
* BES-53248 Node-to-Node-Switch und Switch-to-Switch-Verbindungen müssen Twinax- oder Glasfaserkabel verwenden.
+
...
* Inter-Switch Link (ISL)-Kabel müssen an beiden BES-53248-Switches mit den Ports 0/55 und 0/56 verbunden sein.
* Die erste Anpassung der BES-53248-Switches muss abgeschlossen sein. So werden die:
+
...
(cs1)(Config)# *exit*
----
. Stellen Sie sicher, dass ISL und die physischen Ports auf der ISL zwischen den beiden BES-53248 Switches cs1 und cs2 vorhanden sind `up`:
+
`show port-channel`
...
https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/ONTAP_OS/How_to_suppress_automatic_case_creation_during_scheduled_maintenance_windows["NetApp KB-Artikel: Wie kann die automatische Case-Erstellung während geplanter Wartungszeitfenster unterdrückt werden"^]
