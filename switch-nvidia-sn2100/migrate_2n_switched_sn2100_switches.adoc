...
* Die Nodes müssen ONTAP 9.10.1P3 und höher ausführen.
...
* Bei NVIDIA SN2100 Node-to-Node-Switch und Switch-to-Switch-Verbindungen müssen Twinax- oder Glasfaserkabel verwendet werden.
+
...
* Inter-Switch Link (ISL)-Kabel müssen an die Anschlüsse swp15 und swp16 auf beiden NVIDIA SN2100-Switches angeschlossen werden.
* Die ursprüngliche Anpassung der beiden SN2100-Switches muss abgeschlossen sein. So werden die:
+
...
----
. Stellen Sie sicher, dass sich die ISL und die physischen Ports auf der ISL zwischen den beiden SN2100-Switches sw1 und sw2 auf den Ports swp15 und swp16 befinden: `net show interface`
+
...
----
+
Das folgende Beispiel zeigt, dass die ISL-Ports auf Switch sw2 aktiv sind:
...
----
. Vergewissern Sie sich, dass alle Cluster-Ports aktiv sind: `network port show`
+
...
----
. Vergewissern Sie sich, dass alle Cluster-LIFs betriebsbereit sind und betriebsbereit sind: `network interface show`
+
...
cumulus@sw1:~$ *net commit*
----
. Stellen Sie am Switch sw1 sicher, dass alle Anschlüsse aktiv sind: `net show interface all`
+
[listing, subs="+quotes"]
----
...
cumulus@sw2:~$ *net commit*
----
. Stellen Sie beim Switch sw2 sicher, dass alle Ports aktiviert sind: `net show interface all`
+
[listing, subs="+quotes"]
----
...
...
----
. Vergewissern Sie sich bei beiden Schaltern sw1 und sw2, dass beide Knoten jeweils eine Verbindung zu jedem Switch haben: `net show lldp`
+
...
----
. Zeigen Sie Informationen zu den erkannten Netzwerkgeräten im Cluster an: `net device-discovery show -protocol lldp`
+
[listing, subs="+quotes"]
----
...
----
. Aktivieren Sie das automatische Zurücksetzen auf allen Cluster-LIFs: `net interface modify -vserver Cluster -lif * -auto-revert true`
+
[listing, subs="+quotes"]
----
...
Cluster
          ...
          node2_clus2   true
----
. Vergewissern Sie sich, dass alle Schnittstellen für „true“ anzeigen `Is Home`: `net interface show -vserver Cluster`
+
