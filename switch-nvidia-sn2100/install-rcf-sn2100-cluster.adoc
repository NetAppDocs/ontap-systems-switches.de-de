...
summary: Stellen Sie sicher, dass die folgenden Komponenten auf dem Switch verfügbar sind, bevor Sie das RCF-Skript installieren. 
---
...
* Cumulus Linux ist installiert. Siehe die link:https://hwu.netapp.com/Switch/Index["Hardware Universe"^] für unterstützte Versionen.
* IP-Adresse, Subnetzmaske und Standardgateway werden per DHCP definiert oder manuell konfiguriert.



NOTE: Sie müssen im RCF (zusätzlich zum Administratorbenutzer) einen Benutzer angeben, der speziell für die Protokollerfassung verwendet werden soll.
...
| Cluster | Bei Ports, die für 4x10GbE-Breakout konfiguriert sind, ist ein Port für 4x25GbE-Breakout konfiguriert und die anderen Ports sind für 40/100GbE konfiguriert. Unterstützt gemeinsam genutzten Cluster-/HA-Datenverkehr auf Ports für Knoten, die gemeinsam genutzte Cluster-/HA-Ports verwenden. Siehe die Plattformtabelle im Knowledge Base Artikel link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/What_AFF_ASA_and_FAS_platforms_use_shared_Cluster_and_HA_Ethernet_ports["Welche AFF, ASA und FAS -Plattformen verwenden gemeinsam genutzte Cluster- und HA-Ethernet-Ports?"^]. Alle Ports können auch als dedizierte Cluster-Ports verwendet werden. 
...
Für Cluster- und Speicheranwendungen stehen zwei RCF-Skripte zur Verfügung.  RCFs herunterladen von https://mysupport.netapp.com/site/info/nvidia-cluster-switch["NVIDIA SN2100 Software-Download"^] Seite.  Die Vorgehensweise ist für alle Fälle gleich.
...
. Verwenden Sie die `ping` Befehl zum Überprüfen der Verbindung zum Server, auf dem Cumulus Linux und RCF gehostet werden.
...
====
. Die verfügbaren Schnittstellen des SN2100-Switches anzeigen:
+
[listing, subs="+quotes"]
----
...
----
. Kopiere das RCF-Python-Skript auf den Switch.
+
[listing, subs="+quotes"]
----
...
NOTE: Während `scp` Wird im Beispiel verwendet, können Sie Ihre bevorzugte Methode der Dateiübertragung nutzen, zum Beispiel SFTP, HTTPS oder FTP.

. Wenden Sie das RCF-Python-Skript *MSN2100-RCF-v1._x_-Cluster-HA-Breakout-LLDP* an.
+
[listing, subs="+quotes"]
----
...
NOTE: Im obigen Schritt 3 *Aktualisieren der MOTD-Datei* wird der Befehl verwendet. `cat /etc/motd` wird ausgeführt.  Dies ermöglicht es Ihnen, den RCF-Dateinamen, die RCF-Version, die zu verwendenden Ports und andere wichtige Informationen im RCF-Banner zu überprüfen.

+

NOTE: Bei Problemen mit RCF-Python-Skripten, die nicht behoben werden können, wenden Sie sich bitte an [Kontaktinformationen einfügen]. https://mysupport.netapp.com/["NetApp Support"^] um Unterstützung zu erhalten.

. Wenden Sie alle zuvor vorgenommenen Anpassungen auf die Switch-Konfiguration erneut an.  Siehelink:cabling-considerations-sn2100-cluster.html["Überprüfung der Verkabelung und Konfigurationsüberlegungen"] Einzelheiten zu etwaigen weiteren erforderlichen Änderungen.
...
e3b       Cluster      Cluster          up   9000  auto/10000 healthy  false
----
.. Überprüfen Sie den Zustand des Switches vom Cluster aus (dabei wird möglicherweise Switch sw2 nicht angezeigt, da LIFs nicht auf e0d liegen).
+
[listing, subs="+quotes"]
----
...
. Wiederholen Sie die Schritte 1 bis 14 am zweiten Schalter.
...
NOTE: Im obigen Schritt 3 *Aktualisieren der MOTD-Datei* wird der Befehl verwendet. `cat /etc/issue.net` wird ausgeführt.  Dies ermöglicht es Ihnen, den RCF-Dateinamen, die RCF-Version, die zu verwendenden Ports und andere wichtige Informationen im RCF-Banner zu überprüfen.
...
NOTE: Die aufgeführten Ausnahmen haben keinen Einfluss auf die Leistung und können getrost ignoriert werden.
...
NOTE: Obwohl `scp` Wird im Beispiel verwendet, können Sie Ihre bevorzugte Methode der Dateiübertragung nutzen, zum Beispiel SFTP, HTTPS oder FTP.
...
NOTE: Im oben genannten Schritt 3 *Aktualisieren der MOTD-Datei* wird der Befehl `cat /etc/issue.net` ausgeführt.  Dies ermöglicht es Ihnen, den RCF-Dateinamen, die RCF-Version, die zu verwendenden Ports und andere wichtige Informationen im RCF-Banner zu überprüfen.
...
====
.Wie geht es weiter?
Nach der Installation des RCF können Sie link:setup-install-cshm-file.html["Installieren Sie die CSHM-Datei"]Die
