...
---
= Installieren oder aktualisieren Sie das RCF-Skript (Reference Configuration File)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Cumulus Linux ist installiert. Siehe https://hwu.netapp.com/Switch/Index["Hardware Universe"^] Für unterstützte Versionen.
* IP-Adresse, Subnetzmaske und Standard-Gateway über DHCP oder manuell konfiguriert definiert.



NOTE: Sie müssen im RCF (zusätzlich zum Admin-Benutzer) einen Benutzer angeben, der speziell für die Protokollerfassung verwendet werden soll.
...
| Cluster | Bei Ports, die für 4x10GbE-Breakout konfiguriert sind, ist ein Port für 4x25GbE-Breakout und die anderen Ports für 40/100GbE konfiguriert.  Unterstützt gemeinsam genutzten Cluster-/HA-Verkehr auf Ports für Knoten, die gemeinsam genutzte Cluster-/HA-Ports verwenden.  Die Plattformtabelle finden Sie im Knowledge Base-Artikel. https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/What_AFF_ASA_and_FAS_platforms_use_shared_Cluster_and_HA_Ethernet_ports["Welche AFF, ASA und FAS -Plattformen verwenden gemeinsam genutzte Cluster- und HA-Ethernet-Ports?"^] .  Alle Ports können auch als dedizierte Cluster-Ports verwendet werden. 
...
Für Cluster- und Speicheranwendungen stehen zwei RCF-Skripte zur Verfügung. RCFs von Seite herunterladen https://mysupport.netapp.com/site/info/nvidia-cluster-switch["NVIDIA SN2100 Software-Download"^]. Das Verfahren für jedes ist gleich.
...
. Verwenden Sie die  `ping` Befehl zum Überprüfen der Konnektivität mit dem Server, auf dem Cumulus Linux und RCF gehostet werden.
...
====
** Wenn Sie Ihr RCF aktualisieren, müssen Sie für diesen Schritt die automatische Rücksetzung deaktivieren.
** Wenn Sie Ihre Cumulus Linux-Version gerade aktualisiert haben, müssen Sie die automatische Wiederherstellung für diesen Schritt nicht deaktivieren, da sie bereits deaktiviert ist.
...
NOTE: Während `scp` Im Beispiel wird „http://www.sftp.org/docs/file ...

. Wenden Sie das Skript RCF Python an *MSN2100-RCF-v1._x_-Cluster-HA-Breakout-LLDP*.
+
[listing, subs="+quotes"]
----
...
NOTE: In Schritt 3 *Aktualisierung der MOTD-Datei* oben, der Befehl `cat /etc/motd` Wird ausgeführt. Dadurch können Sie den RCF-Dateinamen, die RCF-Version, die zu verwendenden Ports und andere wichtige Informationen im RCF-Banner überprüfen.
...
. Wenden Sie alle vorherigen Anpassungen erneut auf die Switch-Konfiguration an. link:cabling-considerations-sn2100-cluster.html["Prüfen Sie die Verkabelung und Konfigurationsüberlegungen"]Weitere Informationen zu erforderlichen Änderungen finden Sie unter.
...
NOTE: In Schritt 3 *Aktualisieren der MOTD-Datei* oben, der Befehl  `cat /etc/issue.net` ausgeführt wird. Dadurch können Sie den RCF-Dateinamen, die RCF-Version, die zu verwendenden Ports und andere wichtige Informationen im RCF-Banner überprüfen.
...
NOTE: Die aufgeführten Ausnahmen haben keine Auswirkungen auf die Leistung und können sicher ignoriert werden.
...
NOTE: Obwohl `scp` Im Beispiel wird „http://www.sftp.org/docs/file ...
...
NOTE: In Schritt 3 *Aktualisieren der MOTD-Datei* oben wird der Befehl „cat /etc/issue.net“ ausgeführt. Dadurch können Sie den RCF-Dateinamen, die RCF-Version, die zu verwendenden Ports und andere wichtige Informationen im RCF-Banner überprüfen.
