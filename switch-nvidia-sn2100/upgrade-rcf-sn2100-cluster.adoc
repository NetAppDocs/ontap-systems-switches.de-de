...
---
= Aktualisieren Sie das Skript der Referenzkonfigurationsdatei (RCF).
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Führen Sie diese Schritte aus, um das RCF-Skript zu aktualisieren.

.Bevor Sie beginnen
Vor dem Aktualisieren des RCF-Skripts stellen Sie sicher, dass Folgendes auf dem Switch verfügbar ist:
...
NOTE: Obwohl `scp` Wenn im Beispiel eine Methode verwendet wird, können Sie Ihre bevorzugte Methode zur Dateiübertragung verwenden.
...
NOTE: Im obigen Schritt 3 *Aktualisieren der MOTD-Datei* wird der Befehl verwendet. `cat /etc/issue` wird ausgeführt.  Dies ermöglicht es Ihnen, den RCF-Dateinamen, die RCF-Version, die zu verwendenden Ports und andere wichtige Informationen im RCF-Banner zu überprüfen.
...
NOTE: Während `scp` Wenn im Beispiel eine Methode verwendet wird, können Sie Ihre bevorzugte Methode zur Dateiübertragung verwenden.
