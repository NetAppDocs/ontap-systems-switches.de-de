...
Es werden folgende Annahmen angenommen:

* Sie verfügen über ein Linux-Wissen auf mittlerer Ebene.
* Sie kennen die grundlegende Textbearbeitung, UNIX-Dateiberechtigungen und die Prozessüberwachung. Eine Vielzahl von Texteditoren sind vorinstalliert, einschließlich `vi` Und `nano`.
* Sie müssen Zugriff auf eine Linux oder UNIX Shell haben. Wenn Sie Windows verwenden, verwenden Sie eine Linux-Umgebung als Kommandozeilen-Tool für die Interaktion mit Cumulus Linux.
...
.Schritte
. Melden Sie sich beim Switch an. Wenn Sie sich zum ersten Mal am Switch anmelden, benötigen Sie den Benutzernamen/das Passwort von *cumulus*/*cumulus* mit `sudo` Berechtigungen:
+
[listing, subs="+quotes"]
----
...
----
. Erstellen Sie einen neuen Benutzer, und fügen Sie diesen Benutzer dem hinzu `sudo` Gruppieren. Dieser Benutzer wird erst wirksam, wenn die Konsole/SSH-Sitzung neu gestartet wird:
+
[listing, subs="+quotes"]
----
